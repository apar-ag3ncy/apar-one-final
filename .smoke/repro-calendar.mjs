...
try {
  ...
  const dateBtn = modal.locator('button:has(svg.lucide-calendar), button:has-text("Pick a date")').first();
  ...
  if (hasDateBtn) {
    ...
    const pop = page.locator('[data-slot="popover-content"], [role="dialog"], .rdp, [data-slot="calendar"]').last();
    const popHtml = await pop.innerHTML().catch(() => '(no popover)');
    const dayCells = await page.locator('[data-slot="calendar"] button[data-day], .rdp-day, table td button').count();
    const monthCaption = await page.locator('[data-slot="calendar"], .rdp').innerText().catch(() => '');
    ...
    // --- INTERACTION TESTS ---
    const captionOf = async () => (await page.locator('[data-slot="calendar"] .rdp-month_caption, .rdp-caption_label, [data-slot="calendar"]').first().innerText().catch(() => '')).split('\n')[0];
    const cap0 = await captionOf();
    console.log('CAPTION_BEFORE=' + JSON.stringify(cap0));
    // previous-month arrow
    const prevBtn = page.locator('button.rdp-button_previous, button[aria-label*="previous" i], .rdp-nav button').first();
    ...
    if (nSel >= 1) {
      ...
      console.log('YEAR_OPTIONS_SAMPLE=' + JSON.stringify([opts[0], opts[1], opts[opts.length-1]]) + ' count=' + opts.length);
      // pick a far-past year (e.g. 1990) to prove reachability
      const target = opts.find((o) => o.trim() === '1990') ? '1990' : opts[0];
      await yearSel.selectOption({ label: target }).catch(async () => { await yearSel.selectOption(target).catch(()=>{}); });
      await page.waitForTimeout(700);
    ...
    const fieldText = await modal.locator('button:has(svg.lucide-calendar)').first().innerText().catch(() => '');
    console.log('AFTER_DAY_CLICK_field=' + JSON.stringify(fieldText) + ' popoverStillOpen=' + popStillOpen);
