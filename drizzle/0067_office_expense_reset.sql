...
--
-- The office-expense book had accumulated duplicate rows: the OLD importer
-- posted each row in its own transaction and then timed out before returning,
-- so re-running the import re-inserted rows (29 duplicate groups / 61 rows).
-- The operator asked for a clean slate before re-importing a corrected sheet.
--
-- SCOPE — deletes exactly 147 office_expenses rows (all supplies; live +
-- already soft-deleted) plus the 140 posted journals and their postings.
-- Live value removed: Rs 6,21,552.38.
--
-- PRESERVED — the 11 salary payouts inside office_expenses (Rs 7,13,000):
--   * 2026-03-07, category "Feb 26 Salary" (5 rows)
--   * 2026-04-08, uncategorised (6 rows)
-- These are true payroll records and are NOT touched, nor are their journals.
-- The separate salary_payments / salary_structures tables are untouched entirely.
--
-- Rows are addressed by EXPLICIT primary key so this can never over-delete on
-- another database (a NOT IN predicate would wipe an unrelated DB's expenses).
-- Re-running is a no-op.
--
-- Posted ledger rows are delete-protected by trg_*_block_delete (LEDGER-SPEC
-- §0.3: "reverse, never delete"). The operator chose a hard wipe over reversal
-- so the re-imported book has no net-zero reversal noise, so we lift those two
-- triggers for the duration of this migration and restore them after. The
-- balance constraint trigger is unaffected (it no-ops on DELETE). Migrations
-- run in a transaction, so a failure rolls the whole thing back.
--
-- A full JSON backup of every deleted row (expenses + transactions + postings)
-- was taken before authoring this migration.

ALTER TABLE public.transactions DISABLE TRIGGER trg_transactions_block_delete;
--> statement-breakpoint
ALTER TABLE public.postings DISABLE TRIGGER trg_postings_block_delete;
--> statement-breakpoint

DELETE FROM public.postings WHERE transaction_id IN (
...
DELETE FROM public.office_expenses WHERE id IN (
...
ALTER TABLE public.transactions ENABLE TRIGGER trg_transactions_block_delete;
--> statement-breakpoint
ALTER TABLE public.postings ENABLE TRIGGER trg_postings_block_delete;
--> statement-breakpoint

-- Fail (and roll the whole migration back) rather than ever commit with the
-- ledger's delete-protection left disabled. tgenabled 'O' = enabled/origin.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'trg_transactions_block_delete' AND tgenabled = 'O'
  ) OR NOT EXISTS (
    SELECT 1 FROM pg_trigger
    WHERE tgname = 'trg_postings_block_delete' AND tgenabled = 'O'
  ) THEN
    RAISE EXCEPTION 'ledger delete-protection triggers were not re-enabled; aborting';
  END IF;
END $$;
--> statement-breakpoint

-- The 11 salary rows must survive. Guards against a mis-scoped delete list.
DO $$
DECLARE v_count int;
BEGIN
  SELECT count(*) INTO v_count FROM public.office_expenses WHERE deleted_at IS NULL;
  IF v_count <> 11 THEN
    RAISE EXCEPTION 'expected 11 surviving office_expenses (the salary rows), found %', v_count;
  END IF;
END $$;
