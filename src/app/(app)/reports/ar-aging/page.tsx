import type { Metadata } from 'next';
import { ProfileHeader } from '@/components/entity/profile-header';
import { getAgingReport } from '@/lib/server-stub/ledger-actions';
import { AgingClient } from '../_aging/aging-client';

export const metadata: Metadata = { title: 'AR aging · Apar Dashboard' };

type Props = { searchParams: Promise<{ asOf?: string }> };

export default async function ArAgingPage({ searchParams }: Props) {
  const sp = await searchParams;
  const asOfDate = sp.asOf ?? new Date().toISOString().slice(0, 10);
  const rows = await getAgingReport({ side: 'receivable', asOfDate });
  return (
    <>
      <ProfileHeader
        title="AR aging"
        subtitle="Open receivables bucketed by days outstanding. 0-30 / 31-60 / 61-90 / 90+."
        back={{ href: '/reports', label: 'All reports' }}
      />
      <AgingClient side="receivable" rows={rows} asOfDate={asOfDate} basePath="/reports/ar-aging" />
    </>
  );
}
