...
type ClientValues = {
  // Step 1 — Identity
  name: string;
  legalType: 'individual' | 'sole_prop' | 'partnership' | 'llp' | 'pvt_ltd' | 'public_ltd' | '';
  primaryEmail: string;
  primaryPhone: string;
  // Step 2 — Tax & legal
  pan: string;
  ...
  // Step 6 — Prior records & documents (uploaded post-create)
  documents: DocumentDraft[];
  // Step 7 — Custom fields (org-defined via Form Builder)
...
export function ClientWizard() {
  ...
  const steps: WizardStep<ClientValues>[] = [
    {
      id: 'identity',
      title: 'Identity',
      description: 'Legal name, type, primary contacts',
      validate: (v) => {
        const errors: Record<string, string> = {};
        if (!v.name.trim()) errors.name = 'Legal name is required';
        if (!v.legalType) errors.legalType = 'Legal type is required';
      ...
      render: ({ values, onPatch, errors }) => (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Field label="Legal name" error={errors.name} required>
            <Input value={values.name} onChange={(e) => onPatch({ name: e.target.value })} />
          </Field>
          <Field label="Legal type" error={errors.legalType} required>
            <Select
              value={values.legalType}
              onValueChange={(v) => onPatch({ legalType: v as ClientValues['legalType'] })}
            >
              <SelectTrigger>
    ...
    },
    {
      id: 'contacts',
      title: 'Contacts',
      description: 'At least one POC required',
      validate: (v) => {
        const errors: Record<string, string> = {};
        if (v.contacts.length === 0) {
          errors['contacts.0.name'] = 'Add at least one contact';
        }
        v.contacts.forEach((c, idx) => {
          if (!c.name.trim()) errors[`contacts.${idx}.name`] = 'Name required';
          if (!c.email.trim() && !c.phone.trim()) {
            errors[`contacts.${idx}.email`] = 'Email or phone required (amendment §1)';
          }
        });
        return errors;
      },
      ...
      render: ({ values, onPatch }) => (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <Field
            ...
            hint="Stored encrypted; only masked form displayed afterwards"
          >
            <Input
              className="font-mono"
    ...
    },
    {
      id: 'contract',
      title: 'Contract',
      description: 'Server enforces — pending requires reason + ETA',
      validate: (v) => {
        const errors: Record<string, string> = {};
    ...
    },
    {
      id: 'prior-records',
      title: 'Prior records',
      description: 'Previous invoices, contracts, receipts — optional',
      render: ({ values, onPatch }) => (
        <PriorRecordsStep
          entityType="client"
  ...
  async function handleSubmit(values: ClientValues) {
    if (!values.legalType) {
      return {
        ok: false as const,
        message: 'Pick a legal type before saving.',
    ...
    if (documents.some((d) => d.file) || customValues.length > 0) {
      ...
      if (post.failures.length > 0) {
        toast.warning(`Client created, but ${post.failures.length} attachment(s) failed.`, {
          description: `${post.failures.slice(0, 3).join('; ')} — re-add these from the client's Documents tab.`,
          duration: 10000,
        });
