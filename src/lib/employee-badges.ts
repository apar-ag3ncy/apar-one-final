 ...
 * full-time / part-time / intern employees, which meant people carried a
 * probation badge nobody had actually put them on — an intern with no
 * probation date on record still displayed "Probation · 167d left". Now that
 * employees can see their own record in the portal, a status the founder never
 * set must not be shown to them (or in the OS). Clearing the date ends
 * probation; there is no implicit window.
 */
